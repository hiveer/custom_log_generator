namespace :custom_log do
  ...
  task :install do
    ...
    puts "copy lib/initializers/generate_custom_logs.rb to Rails.root/config/initializers ..."
    source_config_file = File.join(Gem.loaded_specs["custom_log_generator"].full_gem_path, "lib", "initializers", "generate_custom_logs.rb")
    target_config_file = File.join(Rails.root, "config", "initializers", "generate_custom_logs.rb")
    FileUtils.cp_r source_config_file, target_config_file
    puts "all done, enjoy!"
  end
end
